...
*/
function deleteOne(var1, var2) {
  if (var2) {
    return var1.slice(1);
  }
  else {
    return var1.slice(0, (var1.length) - 1);
  }
}
...
*/
function onlyLetters(var5) {
  let lettere = var5.replace(/[0-9]/g, '');
  return lettere
}
console.log(onlyLetters('oggi è Venerdì 17'));
/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/
function isThisAnEmail(var6) {
  if (var6.includes('@') && var6.includes('.')) {
    return true;
  }
  else return false;
}
console.log(isThisAnEmail('chiaraandria150301@gmail.com'));
/* ESERCIZIO 7
...
function isTodayMyBirthday() {
  let oggi = new Date();
  if (oggi.getDate() == 15 && oggi.getMonth() == 2) {
    return true;
  }
  else return false;
}
...
*/
function deleteProp(pippo, pluto) {
  delete pippo[pluto];
  return pippo;
}
...
*/
function searchByTitle(var7) {
  let = ricercatoFilm = [];
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.toLocaleLowerCase().includes(var7)) {
      ricercatoFilm.push(movies[i].Title);
    }
    else {
      console.log('Non ho trovato nessun film!');
    }
  }
  return ricercatoFilm
}
...
*/ /*function addLiElement() {
  ...
  let textLi = document.createTextNode('sesto').innerHTML;
  ul.appendChild(li);
